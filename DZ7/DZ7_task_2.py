...
# Здесь пишем код
class PersonInfo:
    def __init__(self, fio: str, age: int, *deps):
        self.fio = fio
        self.age = age
        self.deps = deps
    ...
    def short_name(self):
        """
        Перевод из "Имя Фамилия" в "Фамилия И."
        :return: Строка ""Фамилия И."
        """
        list_name = self.fio.split(' ')
        lastname = list_name[1]
        short_name = list_name[0][0]
        return f'{lastname} {short_name}.'

    def path_deps(self):
        """
        Получаем путь по подразделениям с разделителем "-->"
        :return: Строка подразделений
        """
        return ' --> '.join(self.deps)

    def new_salary(self):
        """
        Вычисление новой зарплаты
        :return: Новая зарплата
        """
        deps_str = ''.join(self.deps)
        set_str = set([deps_str[i] for i in range(len(deps_str))])
        char_count = sorted([deps_str.count(i) for i in set_str], reverse=True)
        return 1337 * self.age * sum(char_count[: 3])
