...
def scrabble(word):
    ...
    """
    Функция, считает кол-во очков (points), полученное за слово (word) согласно "цены" каждой буквы и выводит сумму за переданное в строке (слово, выражение).
    :param word - строка на вход (слово или группа слов)
    :return: сумма букв слова/слов - согласно указанной "цены" за каждую букву
    """
    points = 0 #Для подсчета очков, на старте их нуль
    print("word:", word)
    dictionary_points = {1: 'авеёинорст', 2: 'дклмпу', 3: 'бгья', 4: 'йы', 5: 'жзхцч', 8: 'фшэю', 10: 'щ', 15: 'ъ'}
    for x in word:
        for key, value in dictionary_points.items():
            if x in value:
                points = points + key
    print("Очков за слово:", points)
    return points
