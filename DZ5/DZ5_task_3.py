...
def everything_for_your_cat(cats_data):
    """
    Принимает на вход список кортежей "Кличка котика, Возраст котика, Имя покупателя, Фамилия покупателя."
    Упорядочивает входящие данные, возвращает стоку.
    :param cats_data - список кортежей с данными хозяев и питомцев
    :return: Строка из владельцев и котов в формате "Имя_покупателя Фамилия_покупателя: Кличка_котика1, Возраст; ...; Кличка_котикаN, Возраст\n"
    """
    # Здесь нужно написать код
    dictionary = dict ()
    for x in range(len(cats_data)):
        man = ' '.join(map(str, cats_data[x][2:]))
        cat = ', '.join(map(str, cats_data[x][:2]))
        dictionary[man] = f'{dictionary[man]}; {cat}' if dictionary.get(man) else cat
    our_str = str()
